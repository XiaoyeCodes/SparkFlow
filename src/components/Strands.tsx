...
type LiveProps = Required<Omit<StrandsProps, 'className' | 'style'>>;

const MAX_STRANDS = 12;
const MAX_COLORS = 8;

const defaultLiveProps: LiveProps = {
...
export function Strands(props: StrandsProps) {
  const liveProps = { ...defaultLiveProps, ...props };
  const { className = '', style } = props;
  const propsRef = useRef<LiveProps>(liveProps);
  ...
  useEffect(() => {
    ...
    const update = (time: number) => {
      ...
      program.uniforms.uColors.value = buildPalette(latest.colors);
      program.uniforms.uColorCount.value = Math.min(latest.colors.length, MAX_COLORS);
      program.uniforms.uStrandCount.value = Math.min(Math.max(Math.round(latest.count), 1), MAX_STRANDS);
      program.uniforms.uSpeed.value = latest.speed;
      program.uniforms.uAmplitude.value = latest.amplitude;
      program.uniforms.uWaviness.value = latest.waviness;
      program.uniforms.uThickness.value = latest.thickness;
      program.uniforms.uGlow.value = latest.glow;
      program.uniforms.uTaper.value = latest.taper;
      program.uniforms.uSpread.value = latest.spread;
      program.uniforms.uHueShift.value = latest.hueShift;
      program.uniforms.uIntensity.value = latest.intensity;
      program.uniforms.uOpacity.value = latest.opacity;
      program.uniforms.uScale.value = latest.scale;
      program.uniforms.uSaturation.value = latest.saturation;

      if (latest.glass) {
        renderer.render({ scene: mesh, target: renderTarget });
        glassProgram.uniforms.uScene.value = renderTarget.texture;
        glassProgram.uniforms.uRefraction.value = latest.refraction;
        glassProgram.uniforms.uDispersion.value = latest.dispersion;
        glassProgram.uniforms.uRadius.value = 0.46 * latest.glassSize;
  ...
  return <div ref={containerRef} className={`strands-container${className ? ` ${className}` : ''}`} style={style} />;
}
