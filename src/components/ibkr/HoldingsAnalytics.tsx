...
import { donutArcPath, chartMoney, chartPercent, holdingsAllocation, holdingsReturnSeries, returnGeometry, type AllocationSlice, type HoldingsRange } from '../../lib/ibkr/holdingsAnalytics';
...
function Donut({ slices, label, center, caption, print, amounts = false, details, innerSlices = [] }: { slices: AllocationSlice[]; label: string; center: string; caption: string; print: boolean; amounts?: boolean; details?: AccountAmount[]; innerSlices?: (AllocationSlice & { displayValue?: number })[] }) {
  ...
  return <div className={`ha-donut-layout${details ? ' ha-amount-donut' : ''}`}><svg viewBox="0 0 200 200" role="img" aria-label={label} className="ha-donut">
    ...
    {total > 0 && slices.map((slice, index) => { const fraction = slice.value / total; const start = offset; offset += fraction; return <path key={index} d={donutArcPath(start, fraction, doubleRing ? 84 : 76)} fill="none" stroke={slice.color} strokeWidth={doubleRing ? active === index ? 20 : 17 : active === index ? 29 : 24} strokeLinecap="butt" opacity={active === null || active === index ? 1 : 0.4} onMouseEnter={() => { setActive(index); setActiveDetail(null); setActiveInner(null); }} onMouseLeave={() => setActive(null)}><title>{slice.label} · {chartMoney(slice.value)} · {(fraction * 100).toFixed(1)}%</title></path>; })}
    {doubleRing && innerSlices.map((slice, index) => {
      const fraction = slice.value / innerTotal, start = innerOffset; innerOffset += fraction;
      return <path className="ha-funding-segment" key={slice.label} d={donutArcPath(start, fraction, 63)} fill="none" stroke={slice.color} strokeWidth={activeInner === index ? 17 : 14} strokeLinecap="butt" onMouseEnter={() => { setActiveInner(index); setActiveDetail(null); setActive(null); }} onMouseLeave={() => setActiveInner(null)}><title>{slice.label} · {chartMoney(slice.displayValue ?? slice.value)} · {(fraction * 100).toFixed(1)}%</title></path>;
    })}
