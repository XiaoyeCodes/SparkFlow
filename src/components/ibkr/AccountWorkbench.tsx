...
const readableConnectionDetail = (detail: string) => detail === 'MCP_AUTHORIZATION_REQUIRED' ? '官方 MCP 授权尚未完成或已经失效，请重新连接 IBKR 后同步当前账户。' : detail;
...
export function AccountWorkbench() {
  ...
  const act = async (name: string, endpoint: string, payload: unknown = {}) => {
    ...
    try { await request(endpoint, payload); await refresh(); return true; } catch (e) { const message = e instanceof Error ? e.message : '操作失败'; try { await refresh(); } catch { /* Preserve the action error when refreshing state also fails. */ } setError(message); return false; } finally { setBusy(''); }
  };
  ...
  const snapshot=state?.snapshot; const latest=[...(state?.reports??[])].filter(r=>r.kind!=='chat').sort((a,b)=>Date.parse(b.generatedAt)-Date.parse(a.generatedAt))[0]; const holding=snapshot?.positions.find(p=>p.conId===selected);
  ...
  {tab==='orders'&&<PaperTradingWorkspace key={state.snapshot.accountKey} state={state} openSettings={()=>navigate('settings')}/>}
  {tab==='backtests'&&<BacktestWorkspace holdings={state.snapshot.positions}/>}
  {tab==='reports'&&<AnalysisWorkspace state={state} report={activeReport} focusText={riskFocus?.reportId===activeReport?.id?riskFocus?.text:undefined} task={task?.state==='completed'?undefined:task} running={running} busy={Boolean(busy)} canAnalyze={canAnalyze} question={question} onQuestion={setQuestion} onAsk={parentReportId=>{const submitted=question;if(submitted.trim())void act('chat','analyze',{question:submitted,parentReportId}).then(ok=>{if(ok)setQuestion(value=>value===submitted?'':value);});}} onStart={()=>void act('analyze','analyze')} onTaskAction={job=>void act('job',job.state==='running'?'cancel':(job.progress?.modelCalls??0)===0?'resume':'retry',{id:job.id})} onRevalidate={job=>void act('job','revalidate',{id:job.id})} onSelectTask={setSelectedJobId} onSelect={r=>{setRiskFocus(undefined);setReportId(r.id);}} onCloseReport={()=>{setRiskFocus(undefined);setReportId(undefined);}} onPlan={()=>{setChosenAlert(undefined);setChosenPlan(undefined);setPlanReportId(activeReport?.id);navigate('alerts');}}/>}
  ...
  {tab==='settings'&&<WorkbenchSettings state={state} busy={busy} act={async (name, endpoint, payload) => { await act(name, endpoint, payload); }} connect={connect} sourceChoice={sourceChoice} selectSource={selectSource}/>}</>}
  {tab!=='overview'&&tab!=='holdings'&&tab!=='valuation'&&tab!=='reports'&&<footer className="awb-footer"><span><ShieldCheck size={13}/>{state?.source==='gateway'&&state.gatewayMode==='paper'?'模拟账户 · 每笔下单单独确认':'真实账户只读'} · 操作建议由你审核</span><span>本机服务运行期间持续更新</span></footer>}</div>
...
function ConnectionSettings({ state, busy, act, connect, sourceChoice, selectSource }: { state: WorkbenchState; busy: string; act: (name: string, endpoint: string, data?: unknown) => Promise<void>; connect: (choice: SourceChoice) => void; sourceChoice: SourceChoice | null; selectSource: (source: SourceChoice['source'], gatewayMode?: SourceChoice['gatewayMode']) => void }) {
  ...
  const mcpDetail = readableConnectionDetail(state.connection.detail);
  const copy = phase === 'connected'
    ? { kicker: 'SECURE CHANNEL ONLINE', title: 'IBKR 已连接', badge: '已连接', detail: mcpDetail || '只读账户通道工作正常。' }
    : phase === 'connecting'
      ? { kicker: 'AUTHENTICATING', title: '正在建立安全连接', badge: '连接中', detail: '正在等待 IBKR 官方授权响应，请在新窗口完成登录。' }
      : { kicker: 'CHANNEL OFFLINE', title: authorized ? '授权需要恢复' : '尚未连接 IBKR', badge: authorized ? '需要重连' : '未连接', detail: !viewingActive ? '当前连接保持不变；点击连接后再确认是否切换至官方 MCP。' : mcpDetail || '连接后才能读取真实持仓与账户快照。' };
