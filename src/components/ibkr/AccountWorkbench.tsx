...
export function AccountWorkbench() {
  ...
  type Tab = 'overview' | 'reports' | 'valuation' | 'alerts' | 'holdings' | 'settings';
  const [tab, setTab] = useState<Tab>(() => { const value = searchParams.get('tab'); return ['overview','reports','valuation','alerts','holdings','settings'].includes(value ?? '') ? value as Tab : 'overview'; });
  useEffect(() => { const value = searchParams.get('tab'); if (['overview','reports','valuation','alerts','holdings','settings'].includes(value ?? '')) setTab(value as Tab); }, [searchParams]);
  ...
  const navItems=[['overview','账户总览',Home],['reports','AI 分析',ChartNoAxesCombined],['valuation','大盘估值',Gauge],['alerts','机会与风险',ShieldCheck],['holdings','持仓',BriefcaseBusiness],['settings','设置',Settings2]] as const;
  const titles={valuation:'大盘估值分位监控',overview:'账户总览',reports:'投资组合分析报告',alerts:'机会与风险中心',holdings:'我的持仓',settings:'连接账户与分析偏好'};
  const subtitles={valuation:'指数趋势、估值与情绪的透明规则评分',overview:'你的资产、市场与下一步',reports:'完整账户数据一次提交、一次分析、一个结论',alerts:'把市场变化转化为可执行的计划',holdings:'持仓结构与组合风险敞口',settings:'管理账户连接、研究授权与投资约束'};
  ...
  {tab==='settings'&&<WorkbenchSettings state={state} busy={busy} act={act} connect={connect}/>}</>}
...
function ConnectionSettings({ state, busy, act, connect }: { state: WorkbenchState; busy: string; act: (name: string, endpoint: string, data?: unknown) => Promise<void>; connect: () => Promise<void> }) {
  ...
  return <section className="awb-panel awb-connection-panel">
    ...
    <div className="awb-source-options"><button aria-pressed={state.source === 'mcp'} disabled={!!busy} onClick={() => void act('source', 'source', { source: 'mcp' })}><b>官方 MCP</b><small>网页授权 · 默认方案</small></button><button aria-pressed={state.source === 'gateway'} disabled={!!busy} onClick={() => void act('source', 'source', { source: 'gateway' })}><b>TWS / Gateway</b><small>本机客户端 · 原有只读服务</small></button></div>
...
function WorkbenchSettings({ state, busy, act, connect }: { state: WorkbenchState; busy: string; act: (name: string, endpoint: string, data?: unknown) => Promise<void>; connect: () => Promise<void> }) {
  ...
  return <div className="awb-settings-grid"><ConnectionSettings state={state} busy={busy} act={act} connect={connect} />
