...
function Canvas({data}:{data:TicketHistory}){
  ...
  useEffect(()=>{
    ...
    setHover(null);
    const rows=data.bars.map(bar=>({...bar,label:bar.time,time:intraday?Date.parse(bar.time.replace(' ','T')+':00+08:00')/1000 as UTCTimestamp:bar.time as Time}));
    const opening=data.bars[0]?.open;
    hoverRows.current=new Map(rows.map((bar,index)=>{
      const reference=intraday||index===0?opening:data.bars[index-1]?.close;
    ...
    if(intraday)(series.current as ISeriesApi<'Area'>).setData(rows.map(bar=>({time:bar.time,value:bar.close})));
    ...
    if(!fitted.current){
      if(!intraday&&rows.length>90)api.current?.timeScale().setVisibleLogicalRange({from:rows.length-90,to:rows.length+3});
  ...
  return <><div className="pt-chart-hover">移动鼠标查看行情详情 · 拖动或滚轮缩放</div><div className="pt-chart-stage">
    <div className="pt-chart-canvas" ref={host} role="img" aria-label={`${data.symbol} ${periods.find(p=>p[0]===data.period)?.[1]}走势图，共 ${data.bars.length} 个数据点`}/>
...
export function TicketPriceChart({contract}:{contract?:PaperContract}){
  ...
  return <section className="pt-chart-panel" aria-label="股票走势图"><div className="pt-chart-toolbar"><div role="tablist" aria-label="行情周期">{periods.map(([value,label])=><button key={value} role="tab" aria-selected={period===value} onClick={()=>setPeriod(value)}>{label}</button>)}</div><span>{period==='intraday'?'北京时间': `${data?.adjustment||'历史行情'} · USD`}</span></div>
