...
import { paperFilledQuantity, paperReceiptStatus, type PaperReceipt } from '../../lib/ibkr/paperReceipt';
import { paperPeFact } from '../../lib/ibkr/paperValuation';
import { startQuotePolling } from '../../lib/realtimeQuotes';
import { beijingDateKey, paperHoldingValues, paperOrdersForBeijingDay, paperOrderTime, paperSortNumber, sortPaperTable, type PaperTableSort } from '../../lib/ibkr/paperTableSort';
...
const errors: Record<string, string> = {
  ...
  STALE_ACCOUNT_CHECKPOINT: '账户安全检查点已过期，系统正在从 IBKR 重新核对现金与持仓；完成后请重新预览。',
  ACCOUNT_INTEGRITY_HALT: '订单记录需要通过券商账户重新核对，系统会自动重试。若持续失败，请保留当前订单状态以便检查。',
  SDK_NOT_READY: 'IBKR 与交易服务器的连接正在恢复，订单尚未发送；连接恢复后请重新预览。',
  BRIDGE_RESTARTING: '正在更新本地桥接服务，请连接恢复后重新预览。',
  OUTSIDE_RTH: '本地桥接仍在使用旧的休市风控。请等待自动升级完成后重新预览。',
  DAILY_PNL_UNAVAILABLE: 'Gateway 尚未返回今日盈亏，暂不能完成订单校验。',
  MISSING_ACCOUNT_DATA: 'Gateway 尚未返回完整的美元现金与账户数据。',
  ORDER_NOTIONAL_LIMIT: '本地桥接仍在使用旧的单笔金额风控。请等待自动升级完成后重新预览。',
...
const quoteFacts = [
  ['open', '今开', 'price', Sunrise], ['close', '昨收', 'price', History], ['high', '最高', 'price', TrendingUp], ['peTtm', '市盈率（TTM）', 'ratio', Activity],
  ['low', '最低', 'price', TrendingDown], ['volume', '成交量', 'compact', BarChart3], ['amount', '成交额 · USD', 'compact', CircleDollarSign], ['peDynamic', '市盈率（动）', 'ratio', Scale],
] as const;
...
export const sortPaperOrdersNewestFirst = (orders: PaperOrderRecord[]) => [...orders].sort((a, b) => {
  ...
  const aId = a.orderId ?? a.brokerOrderId, bId = b.orderId ?? b.brokerOrderId;
  if (aId != null || bId != null) {
    if (aId == null) return 1;
    if (bId == null) return -1;
    if (aId !== bId) return bId - aId;
  }
  return b.intent.clientIntentId.localeCompare(a.intent.clientIntentId);
});
const orderColumns = [['symbol', '股票'], ['side', '方向'], ['quantity', '委托数量'], ['limitPrice', '限价 / USD'], ['execution', '订单状态'], ['filled', '成交数量'], ['averageFillPrice', '成交均价 / USD'], ['createdAt', '下单时间（北京时间）']];
const holdingColumns = [['symbol', '股票'], ['quantity', '持仓数量'], ['currentPrice', '当前价格'], ['averageCost', '平均成本'], ['unrealizedPnl', '当前盈亏'], ['marketValue', '持仓市值']];
...
export function PaperTradingWorkspace({ state, openSettings }: { state: WorkbenchState; openSettings: () => void }) {
  ...
  const [limits, setLimits] = useState<Record<Exclude<keyof PaperRiskLimits, 'maxOrderNotional'>, string>>(() => ({
    maxTotalExposure: String(Math.max(1, Math.floor(Number(state.snapshot.metrics.netLiquidation || 100000) * .95))),
  ...
  const [orderSort, setOrderSort] = useState<PaperTableSort>({ key: 'createdAt', direction: 'default' });
  const [holdingSort, setHoldingSort] = useState<PaperTableSort>({ key: 'symbol', direction: 'default' });
  const [pages, setPages] = useState({ orders: 1, holdings: 1 });
  const latestPrice = quote && String(quote.conId) === order.conId
  ...
  const syncLabel = status?.connection !== 'connected' ? '等待连接，自动重试'
    : !syncObservedAt && !statusSync.failed ? '正在同步'
    : syncDelayed ? '自动同步延迟'
    : '已同步';
  ...
  const refreshQuote = useCallback(async (contract: PaperContract, quiet = false) => {
    ...
    try { const value = await paperRequest<PaperQuote>('market-quote', { conId:contract.conId,symbol:contract.symbol,currency:contract.currency,exchange:contract.exchange,dataSource }); if (revision === quoteRevision.current && dataSource===dataSourceRef.current) {setQuote(value);setQuoteError('');return value;} }
  ...
  useEffect(() => {
    ...
    setOrder(current => current.orderType === 'LMT' && current.conId === String(quote?.conId) && !current.limitPrice
      ? { ...current, limitPrice: String(Number(latestPrice)) }
      : current);
  }, [latestPrice, order.orderType, quote?.conId]);
  ...
  const selectOrderType = (orderType: 'LMT' | 'MKT') => {
    setPreview(null);
    setOrder(current => ({
      ...current,
      orderType,
      ...(orderType === 'MKT' ? { tradingSession: 'EXTENDED' as const } : {}),
      ...(orderType === 'LMT' && current.orderType !== 'LMT' && latestPrice != null && Number.isFinite(Number(latestPrice)) && Number(latestPrice) > 0
        ? { limitPrice: String(Number(latestPrice)) }
        : {}),
    }));
  };
  ...
  const chooseHeatmapStock = (stock: HeatmapStockSelection) => execute('resolve', async () => {
    ...
    const matches = rows.filter(row => row.currency === 'USD' && normalize(row.symbol) === normalize(stock.code));
    const exchangeMatch = stock.exchange
      ? matches.find(row => row.exchange === stock.exchange
        || stock.exchange === 'NASDAQ' && row.exchange === 'ISLAND'
        || stock.exchange === 'AMEX' && ['ARCA', 'NYSEARCA'].includes(row.exchange || ''))
      : undefined;
    const match = exchangeMatch ?? matches[0];
    if (!match) throw new Error('IBKR 未返回该热力图股票的匹配合约，请检查 Gateway 连接后重试。');
    add({ ...match, name: match.name || stock.name });
  });
  ...
  const makePreview = () => execute('preview', async () => {
    ...
    setStatus(current);
    setPreview(await paperRequest<PaperOrderPreview>('preview', { conId: selected.conId, side: order.side, quantity: order.quantity, ...(order.tradingSession === 'OVERNIGHT' ? { tradingSession: 'OVERNIGHT' as const } : {}), ...(order.orderType === 'MKT' ? { orderType: 'MKT' } : { limitPrice: order.limitPrice }) }));
  });
  ...
  const currentBeijingDay = beijingDateKey(now);
  const todaysOrders = useMemo(() => paperOrdersForBeijingDay(status?.orders ?? [], now), [status?.orders, currentBeijingDay]);
  const orderedOrders = useMemo(() => sortPaperTable(sortPaperOrdersNewestFirst(todaysOrders), orderSort, (row, key) => {
    switch (key) {
      case 'symbol': return contracts.find(r => r.conId === row.intent.conId)?.symbol || snapshot.positions.find(r => r.conId === row.intent.conId)?.symbol || '合约 ' + row.intent.conId;
      case 'side': return row.intent.side === 'BUY' ? '买入' : '卖出';
      case 'quantity': return paperSortNumber(row.intent.quantity);
      case 'limitPrice': return row.intent.orderType === 'MKT' ? null : paperSortNumber(row.intent.limitPrice);
      case 'execution': return executionNames[row.execution] || submissionNames[row.submission] || '等待券商回报';
      case 'filled': return paperSortNumber(paperFilledQuantity(row));
      case 'averageFillPrice': return paperSortNumber(row.averageFillPrice);
      default: return Date.parse(row.createdAt || '');
    }
  }), [todaysOrders, orderSort, contracts, snapshot.positions]);
  const holdingRows = useMemo(() => snapshot.positions.map(row => ({ ...row, ...paperHoldingValues(row) })), [snapshot.positions]);
  const orderedHoldings = useMemo(() => sortPaperTable(holdingRows, holdingSort, (row, key) => {
    if (key === 'symbol') return row.symbol;
    return paperSortNumber(row[key as 'quantity' | 'averageCost' | 'marketValue' | 'currentPrice' | 'unrealizedPnl']);
  }), [holdingRows, holdingSort]);
  const activeSort = tableTab === 'orders' ? orderSort : holdingSort;
  const totalRows = tableTab === 'orders' ? orderedOrders.length : orderedHoldings.length;
  const pageCount = Math.max(1, Math.ceil(totalRows / 10));
  const currentPage = Math.min(pages[tableTab], pageCount);
  const pageStart = (currentPage - 1) * 10;
  const changeSort = (sort: PaperTableSort) => {
    (tableTab === 'orders' ? setOrderSort : setHoldingSort)(sort);
    setPages(previous => ({ ...previous, [tableTab]: 1 }));
  };
  const sortableHeader = ([key, label]: string[]) => <th key={key} aria-sort={activeSort.key === key && activeSort.direction !== 'default' ? (activeSort.direction === 'asc' ? 'ascending' : 'descending') : 'none'}>
    <button className="pt-sort-heading" onClick={() => changeSort({ key, direction: activeSort.key !== key || activeSort.direction === 'default' ? 'asc' : activeSort.direction === 'asc' ? 'desc' : 'default' })}>{label}<span aria-hidden="true">{activeSort.key === key && activeSort.direction !== 'default' ? (activeSort.direction === 'asc' ? '↑' : '↓') : '↕'}</span></button>
  </th>;
    ...
    <div className="pt-trade-grid">
      <section className="awb-panel pt-market">
        ...
        <div className="pt-stock-heading"><div className="pt-stock-mark">{selected ? <HoldingLogo holding={{ ...selected, assetType: 'STK' }}/> : '—'}</div><div><h2>{selected?.symbol || '选择一只股票'}<span>{selected?.exchange || 'SMART'}</span></h2><p>{selected?.name || '搜索股票代码，查看行情并填写订单'}</p></div><div className="pt-quote-actions"><div className="pt-source-switch" role="group" aria-label="行情数据源">{([['tencent','腾讯'],['eastmoney','东财']] as const).map(([value,label])=><button key={value} type="button" aria-pressed={dataSource===value} disabled={!!busy} onClick={()=>{
          ...
          ++quoteRevision.current;dataSourceRef.current=value;setQuote(null);setQuoteError('');setPreview(null);setDataSource(value);
          try{localStorage.setItem('sparkflow:paper-data-source',value);}catch{}
        }}>{label}</button>)}</div><button className="pt-icon-button" aria-label="刷新股票行情" disabled={!selected || quoteBusy} onClick={() => selected && void refreshQuote(selected)}><RefreshCw size={17} className={quoteBusy ? 'pt-spin' : ''}/></button></div></div>
        ...
        <div className="pt-market-facts pt-chart-facts">{quoteFacts.map(([field,label,kind,Icon])=>{
          const pe = kind === 'ratio' ? paperPeFact(quote, field === 'peDynamic' ? dataSource === 'eastmoney' ? 'static' : 'dynamic' : 'ttm') : null;
          const factLabel = pe?.label ?? label;
          const display = pe ? pe.value == null ? '未提供' : formatQuoteFact(pe.value, kind) : formatQuoteFact(quote?.[field],kind);
      ...
      <section className={'awb-panel pt-ticket ' + (order.side === 'SELL' ? 'is-sell' : '')}>
        <div className="pt-ticket-heatmap">
          <div className="pt-mini-heatmap-heading"><span><BarChart3 size={17}/>美股迷你热力图</span><small>市值面积 · 涨跌幅颜色</small></div>
          <div className="pt-mini-heatmap"><UsMarketHeatmap compact onStockSelect={chooseHeatmapStock}/></div>
        </div>
        <div className="pt-ticket-form">
        ...
        <div className="pt-fixed-field"><span>订单类型</span><div className="pt-order-types" role="group" aria-label="订单类型">{(['LMT','MKT'] as const).map(type => <button key={type} disabled={type === 'MKT' && !status?.supportedOrderTypes?.includes('MKT')} title={type === 'MKT' && !status?.supportedOrderTypes?.includes('MKT') ? '重启本地桥接后即可使用市价单' : undefined} aria-pressed={order.orderType === type} onClick={() => selectOrderType(type)}>{type === 'LMT' ? '限价单' : '市价单'}</button>)}</div></div>
        <div className="pt-fixed-field"><span>交易时段</span><div className="pt-order-types" role="group" aria-label="交易时段"><button aria-pressed={order.tradingSession === 'EXTENDED'} onClick={() => updateOrder({ tradingSession: 'EXTENDED' })}>常规＋盘前盘后</button><button aria-pressed={order.tradingSession === 'OVERNIGHT'} onClick={() => updateOrder({ tradingSession: 'OVERNIGHT', orderType: 'LMT' })}>夜盘</button></div></div>
        <p className="pt-field-help">{order.tradingSession === 'OVERNIGHT' ? '夜盘使用 IBKR OVERNIGHT 路由，仅支持符合条件的美股/ETF限价单。' : outsideRegularHours ? order.orderType === 'MKT' ? '已允许在非常规时段提交市价单；此时流动性较低，IBKR 可能挂起或拒绝该订单。' : '已允许在非常规时段提交限价单；符合 IBKR 和交易所条件时可在盘前盘后成交。' : order.orderType === 'MKT' ? '按市场可成交价格执行，不设成交价上限；资金按参考价加 5% 预留。' : order.side === 'BUY' ? '只以设定价格或更低价格买入。' : '只以设定价格或更高价格卖出已有持仓。'}</p>
        ...
        <div className="pt-order-meta"><div className="pt-fixed-field"><span>有效期</span><b>当日有效 <small>DAY</small></b></div><div className="pt-fixed-field"><span>路由</span><b>{order.tradingSession === 'OVERNIGHT' ? 'IBKR 夜盘' : '含盘前盘后'}</b></div></div>
        ...
        <p className="pt-submit-note">{status?.enabled ? '可自由切换已核验股票；预览后再次确认，才会发送订单。' : '启用模拟交易：' + (selected?.symbol || '所选股票') + ' · 单笔金额不设上限 · ' + hours + ' 小时。预览不会发送订单。'}</p>
        </div>
      </section>
    </div>
    <section className="awb-panel pt-activity"><div className="pt-activity-header"><div role="tablist" aria-label="交易记录"><button role="tab" aria-selected={tableTab === 'orders'} onClick={() => setTableTab('orders')}>今日订单 <span>{todaysOrders.length}</span></button><button role="tab" aria-selected={tableTab === 'holdings'} onClick={() => setTableTab('holdings')}>持仓 <span>{snapshot.positions.length}</span></button></div><span className={`pt-auto-sync ${syncDelayed?'is-delayed':'is-live'}`} title={syncObservedAt ? `最近成功读取：${new Date(syncObservedAt).toLocaleString('zh-CN',{hour12:false})}` : '尚未完成首次同步'}><span className="pt-sync-dot" aria-hidden="true"/><span className="pt-sync-label">{syncLabel}</span>{status?.connection === 'connected' && <small>· 每 2 秒</small>}</span></div>
      <div className="awb-table-scroll">{tableTab === 'orders' ? <table><thead><tr>{orderColumns.map(sortableHeader)}<th>操作</th></tr></thead><tbody>{orderedOrders.slice(pageStart, pageStart + 10).map(row => <tr key={row.intent.clientIntentId}><td><div className="pt-stock-cell">{logoFor(row.intent.conId)}<div><b>{nameFor(row.intent.conId)}</b><small>{(row.orderId ?? row.brokerOrderId) ? '#' + (row.orderId ?? row.brokerOrderId) : '等待订单编号'}</small></div></div></td><td className={row.intent.side === 'BUY' ? 'pt-up' : 'pt-down'}>{row.intent.side === 'BUY' ? '买入' : '卖出'}</td><td>{row.intent.quantity} 股</td><td>{row.intent.orderType === 'MKT' ? '市价' : money(row.intent.limitPrice)}</td><td><span className={'pt-order-state ' + (row.execution === 'FILLED' ? 'filled' : '')}>{executionNames[row.execution] || submissionNames[row.submission] || '等待券商回报'}</span></td><td>{paperFilledQuantity(row)}</td><td>{money(row.averageFillPrice)}</td><td className="pt-order-time">{paperOrderTime(row.createdAt)}</td><td><div className="pt-order-actions"><button onClick={() => viewReceipt(row)}>查看回执</button><button disabled={!!busy || ['FILLED', 'CANCELLED', 'REJECTED'].includes(row.execution)} onClick={() => setCancelOrder(row)}>撤单</button></div></td></tr>)}</tbody></table> : <table><thead><tr>{holdingColumns.map(sortableHeader)}<th>操作</th></tr></thead><tbody>{orderedHoldings.slice(pageStart, pageStart + 10).map(row => <tr key={row.conId}><td><div className="pt-stock-cell"><HoldingLogo holding={{ ...row, assetType: row.assetType || 'STK' }}/><div><b>{row.symbol}</b><small>{row.currency}</small></div></div></td><td>{row.quantity} 股</td><td title="按券商持仓市值 ÷ 持仓数量折算">{money(row.currentPrice)}</td><td>{money(row.averageCost)}</td><td className={row.unrealizedPnl == null || row.unrealizedPnl === 0 ? undefined : row.unrealizedPnl > 0 ? 'pt-up' : 'pt-down'}>{row.unrealizedPnl != null && row.unrealizedPnl > 0 ? '+' : ''}{money(row.unrealizedPnl)}</td><td>{money(row.marketValue)}</td><td><button className="pt-holding-sell" onClick={() => { add({ conId: row.conId, symbol: row.symbol, currency: row.currency }); updateOrder({ side: 'SELL', quantity: String(Math.floor(Number(row.quantity))) }); }}>卖出</button></td></tr>)}</tbody></table>}
      {!(tableTab === 'orders' ? todaysOrders.length : snapshot.positions.length) && <div className="pt-empty"><ShoppingCart size={24}/><div><b>{tableTab === 'orders' ? '今天还没有模拟订单' : '暂无持仓'}</b><p>{tableTab === 'orders' ? '在上方选股票、填写价格与数量，预览并确认后，今天的订单会出现在这里。' : '成交并同步后，持仓会显示在这里。'}</p></div></div>}</div>{totalRows > 10 && <nav className="pt-pagination" aria-label={tableTab === 'orders' ? '订单分页' : '持仓分页'}><span>第 {pageStart + 1}–{Math.min(pageStart + 10, totalRows)} 条，共 {totalRows} 条</span><button disabled={currentPage === 1} onClick={() => setPages(previous => ({ ...previous, [tableTab]: currentPage - 1 }))}>上一页</button><label>第 <select aria-label="页码" value={currentPage} onChange={e => setPages(previous => ({ ...previous, [tableTab]: Number(e.target.value) }))}>{Array.from({ length: pageCount }, (_, index) => <option key={index + 1} value={index + 1}>{index + 1}</option>)}</select> / {pageCount} 页</label><button disabled={currentPage === pageCount} onClick={() => setPages(previous => ({ ...previous, [tableTab]: currentPage + 1 }))}>下一页</button></nav>}<p className="pt-source-note">{tableTab === 'holdings' ? <>价格与金额以各持仓币种计；当前价格按券商持仓市值折算，当前盈亏为未实现盈亏，优先使用券商回报，缺失时按市值减持仓成本计算。</> : <>订单页每天按北京时间自动切换，只显示当天由 SparkFlow 提交的委托；成交状态以 IBKR 回报为准。</>}<br/>模拟资金不影响实盘账户。</p></section>
