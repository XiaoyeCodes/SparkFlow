...
import { accountSchedules, accountScheduleWeekdays } from '../../lib/ibkr/accountSchedules';
import './AccountScheduleSettings.css';

const zoneNames = { 'Asia/Shanghai': '北京时间', 'America/New_York': '纽约时间', UTC: 'UTC' };
const frequencies = [['daily', '每天'], ['weekly', '每周'], ['monthly', '每月']] as const;
...
export function AccountScheduleSettings({ state, busy, save }: { state: WorkbenchState; busy: boolean; save: (value: WorkbenchState['preferences']) => void }) {
  ...
  const invalid = config.times.some(time => !/^(?:[01]\d|2[0-3]):[0-5]\d$/.test(time)) || new Set(config.times).size !== config.times.length
    || frequency === 'weekly' && (!((config.weekday ?? 0) >= 1) || !((config.weekday ?? 0) <= 7))
    || frequency === 'monthly' && (!((config.monthDay ?? 0) >= 1) || !((config.monthDay ?? 0) <= 31));
  const changed = JSON.stringify(config) !== saved;
  const status = state.schedules?.analysis, title = '账户 AI 分析';
  ...
  return <form className="awb-panel awb-schedules" onSubmit={event => { event.preventDefault(); if (!invalid) { const current = accountSchedules(state.preferences); save({ ...state.preferences, daily: false, eventAnalysis: false, maxAutomatic: 0, schedules: { ...current, brief: { ...current.brief, enabled: false }, analysis: config } }); } }}>
    <header><div><h2><Clock3 size={18}/>定时任务</h2><p>设置完整账户分析的执行周期与时间。</p></div><span>仅在本机服务持续运行并到达设定时点时执行</span></header>
    <div className="awb-schedule-grid">
      <section className={`awb-schedule-card ${config.enabled ? 'enabled' : ''}`} aria-label={`${title}定时设置`}>
        ...
        <p>按天、周或月生成完整账户分析，并把最新结论同步展示到账户总览。报告同时保存在「AI 分析」。</p>
        <div className="awb-schedule-fields"><label>执行方式<select aria-label={`${title}执行方式`} value={config.mode} onChange={event => update({ mode: event.target.value as AccountSchedule['mode'] })}><option value="clock">按周期与时间</option><option value="market-close">美股收盘后 30 分钟</option></select></label>
          {config.mode === 'clock' ? <><label>时区<select aria-label={`${title}时区`} value={config.timeZone} onChange={event => update({ timeZone: event.target.value as AccountSchedule['timeZone'] })}>{Object.entries(zoneNames).map(([zone, name]) => <option key={zone} value={zone}>{name}</option>)}</select></label><label>每个执行日次数<select aria-label={`${title}每个执行日次数`} value={config.times.length} onChange={event => {
        ...
        {config.mode === 'clock' && <div className="awb-schedule-recurrence">
          <fieldset><legend><CalendarDays size={14}/>重复周期</legend><div className="awb-schedule-frequency">{frequencies.map(([value, label]) => <label className={frequency === value ? 'active' : ''} key={value}><input type="radio" name="analysis-frequency" aria-label={`${title}${label}执行`} value={value} checked={frequency === value} onChange={() => update({ frequency: value })}/><span>{label}</span></label>)}</div></fieldset>
          {frequency === 'weekly' && <label className="awb-schedule-date-field">每周执行日<select aria-label={`${title}每周执行日`} value={config.weekday ?? 1} onChange={event => update({ weekday: Number(event.target.value) })}>{accountScheduleWeekdays.map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label>}
          {frequency === 'monthly' && <label className="awb-schedule-date-field">每月执行日<select aria-label={`${title}每月执行日`} value={config.monthDay ?? 1} onChange={event => update({ monthDay: Number(event.target.value) })}>{Array.from({ length: 31 }, (_, index) => <option key={index + 1} value={index + 1}>{index + 1} 日</option>)}</select><small>短月没有该日期时，按当月最后一天执行。</small></label>}
        </div>}
    ...
    <div className="awb-schedule-save"><div><p>保存后从下一个符合周期的时点开始。服务关闭期间错过的任务不会补跑，重新启动服务也不会自动生成。</p><p>只有此定时任务到点或你在「AI 分析」中手动发起，才会调用模型。每次通常使用 1 次额度，共享每日总上限 {state.preferences.maxAiCalls} 次（纽约日期）。</p>{invalid && <p role="alert">请填写有效周期与时间，同一定时任务的执行时间不能重复。</p>}</div><button className="primary" type="submit" disabled={busy || invalid || !changed || !state.snapshot.snapshotId}>{busy ? '保存中…' : '保存定时设置'}</button></div>
  </form>;
}
