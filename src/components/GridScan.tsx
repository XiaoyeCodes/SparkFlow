...
export function GridScan({
  ...
  const pushScan = (timeSeconds: number) => {
    ...
    for (let i = 0; i < arr.length && i < MAX_SCANS; i += 1) {
      buf[i] = arr[i];
    }
  ...
  useEffect(() => {
    ...
    const onClick = async () => {
      ...
      if (enableGyro && orientationEvent?.requestPermission) {
        try {
          ...
          // Some browsers expose the permission API but reject outside trusted gestures.
        }
      }
    };
    ...
    if (scanOnClick || enableGyro) el.addEventListener('click', onClick);
  ...
  }, [enableGyro, scanOnClick, snapBackDelay]);
  ...
  useEffect(() => {
    ...
    const tick = () => {
      ...
      const tiltSm = smoothDampFloat(tiltCurrent.current, tiltTarget.current, tiltVel.current, smoothTime, dt);
      tiltCurrent.current = tiltSm.value;
      tiltVel.current = tiltSm.velocity;

      const yawSm = smoothDampFloat(yawCurrent.current, yawTarget.current, yawVel.current, smoothTime, dt);
      yawCurrent.current = yawSm.value;
      yawVel.current = yawSm.velocity;
    ...
    <div ref={containerRef} className={`gridscan${className ? ` ${className}` : ''}`} style={style}>
      {showPreview ? (
        <div className="gridscan__preview">
          ...
          <div className="gridscan__badge">{enableWebcam ? 'Webcam ready' : 'Webcam disabled'}</div>
        </div>
      ) : null}
    </div>
  );
...
function smoothDampFloat(current: number, target: number, velocity: number, smoothTime: number, deltaTime: number) {
  const safeSmoothTime = Math.max(0.0001, smoothTime);
  const omega = 2 / safeSmoothTime;
  const x = omega * deltaTime;
  const exp = 1 / (1 + x + 0.48 * x * x + 0.235 * x * x * x);
  ...
  if (origMinusCurrent * outMinusOrig > 0) {
    out = originalTo;
    return { value: out, velocity: 0 };
  }

  return { value: out, velocity: nextVelocity };
}
