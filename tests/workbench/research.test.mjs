...
function summary(c){return {headline:'今日整体风险关注度：中——持仓集中且经营兑现仍需核验。',briefPoints:['持仓风险来自集中配置，应核对现金约束。','AAPL 公告确认推出新品，订单增长尚未证实。','等待订单兑现；若利润率恶化，应重新评估。'],accountSummary:'配置集中',portfolioRisk:'以程序指标为准',benchmarkComparison:'基准资料不足，不能计算超额收益。',marketContext:'过去一周公告不等于收益增长，其他宏观资料不足。',valuationReview:[{symbol:'AAPL',verdict:'insufficient',rationale:'缺少完整估值口径，暂不判断。',evidenceIds:[]}],opportunities:['订单兑现后再评估'],risks:['利润兑现低于预期'],calendar:[],frameworks:{buffett:{analysis:'先核验长期盈利质量与安全边际。',commentary:'公开价值投资原则下，证据不足时保持耐心。',evidenceIds:[]},peterLynch:{analysis:'新品线索需要收入和利润增速验证。',commentary:'公开成长投资原则下，故事必须由数字兑现。',evidenceIds:[]},rayDalio:{analysis:'集中持仓需要结合流动性与宏观因子管理。',commentary:'公开宏观对冲原则下，先识别共同风险敞口。',evidenceIds:[]}},fullSummary:'核心结论是控制集中度，并等待经营与宏观证据进一步确认。',scenarios:[{name:'base',assumptions:'需求维持',accountImpact:'组合震荡',response:'保持观察'},{name:'upside',assumptions:'订单改善',accountImpact:'权益仓受益',response:'确认后再平衡'},{name:'downside',assumptions:'利润率恶化',accountImpact:'集中风险放大',response:'触发减仓复核'}],targetAllocation:'保留现金缓冲并控制集中度。',monitoring:[{indicator:'利润率',warningLine:'连续恶化',action:'重新评估'}],evidenceIds:c.evidence.slice(0,1).map(e=>e.id),gaps:[],limitations:['缺少长期历史'],disclaimer:'仅供研究，不构成投资建议。'};}
const checkpoint=()=>createResearch(snapshot(),defaults,[],model,true,[]);
test('live pipeline sends recent structured prices, macro observations and publication dates to the one model call',async()=>{
 const c=checkpoint(),base=io(c);let prompt='';const calls=[];
 await runResearch(c,io(c,{
  tool:async(name,args)=>{
   calls.push([name,args.symbol]);
   if(name==='macro')return {url:'https://fred.stlouisfed.org/',series:[{series:'UNRATE',rows:[{date:'2026-08-01',value:4.1}]}]};
   if(name==='prices')return {url:`https://finance.yahoo.com/quote/${args.symbol}/history/`,rows:Array.from({length:65},(_,i)=>({date:new Date(Date.UTC(2026,6,i+1)).toISOString().slice(0,10),close:100+i}))};
   if(name==='read')return {content:'AAPL company quarterly earnings announcement. '.repeat(20),title:'AAPL earnings',publishedAt:'2026-09-14T12:00:00Z'};
   return base.tool(name,args);
  },
  model:async value=>{prompt=value;return base.model(value);},
 }),new AbortController().signal);
 const input=JSON.parse(prompt.split('\n').find(line=>line.startsWith('{"date"')));
 assert.equal(JSON.parse(input.evidence.find(e=>e.title==='AAPL prices').content).rows[0].close,164);
 assert.ok(input.evidence.some(e=>e.title.includes('宏观经济数值')&&e.content.includes('UNRATE')));
 assert.ok(input.evidence.some(e=>e.publishedAt==='2026-09-14T12:00:00.000Z'));
 assert.ok(calls.some(([name,symbol])=>name==='fund'&&symbol==='SPY'));
 assert.ok(!calls.some(([name,symbol])=>name==='financials'&&symbol==='SPY'));
 assert.equal(c.progress.modelCalls,1);
});
...
test('empty provider content is distinct from malformed JSON and truncation',()=>{
 ...
 assert.throws(()=>parseResearchSection({text:'',finishReason:'length'},portfolioSection,[],[]),/OUTPUT_EMPTY/);
 assert.match(parseResearchSection({text:'不符合JSON的分析内容',finishReason:'stop'},portfolioSection,[],[]).rawContent,/分析内容/);
});
...
test('research reads original sources, preserves event mechanism and verifies all holdings',async()=>{const c=checkpoint();let calls=0;const content=await runResearch(c,io(c,{reserve:async()=>{calls++;}}),new AbortController().signal);assert.equal(calls,1);assert.equal(c.progress.complete,true);assert.equal(content.briefPoints.length,3);assert.equal(content.holdings.length,2);assert.equal(content.valuationReview.length,1);assert.equal(content.frameworks.buffett.commentary.includes('公开价值投资原则'),true);assert.match(content.holdings[0].invalidation,/利润率/);assert.ok(c.evidence.every(e=>e.read&&e.fetchedAt&&e.url));assert.ok(c.progress.trace.some(t=>t.tool==='read'));});
test('plain text portfolio output is retained without a second model call',async()=>{const c=checkpoint();let reserved=0,calls=0;const content=await runResearch(c,io(c,{reserve:async()=>{reserved++;},model:async()=>{calls++;return {text:'## 账户分析\n\n现金安排需要结合风险偏好。',finishReason:'stop'};}}),new AbortController().signal);assert.equal(reserved,1);assert.equal(calls,1);assert.equal(c.progress.modelCalls,1);assert.equal(c.progress.maxCalls,1);assert.match(content.rawContent,/现金安排/);assert.ok(content.validationWarnings.length);assert.equal(c.progress.complete,true);});

test('all twelve holdings reach one CIO portfolio call without twelve individual writeups',async()=>{const c=checkpoint();for(let i=3;i<=12;i++){c.snapshot.positions.push({...c.snapshot.positions[0],conId:i,symbol:`STK${i}`});c.targets.push(`STK${i}`);}c.snapshot.positions.forEach(h=>h.accountKey='PRIVATE_ACCOUNT');let calls=0;const base=io(c);const report=await runResearch(c,io(c,{model:async prompt=>{calls++;assert.ok(!prompt.includes('PRIVATE_ACCOUNT'));assert.match(prompt,/一次综合分析/);assert.match(prompt,/完整账户快照/);assert.match(prompt,/今日整体风险关注度/);assert.match(prompt,/前几大持仓/);assert.match(prompt,/未来7–14天/);assert.match(prompt,/巴菲特式价值投资/);assert.match(prompt,/彼得·林奇式成长股/);assert.match(prompt,/雷·达利欧式宏观对冲/);assert.match(prompt,/全文总结/);assert.match(prompt,/行业轮动/);assert.match(prompt,/AI发展进展/);assert.match(prompt,/不是硬性格式约束/);for(const s of c.targets)assert.ok(prompt.includes(s));return base.model(prompt);}}),new AbortController().signal);assert.equal(calls,1);assert.equal(c.progress.maxCalls,1);assert.equal(report.holdings.length,2);assert.equal(report.scenarios.length,3);assert.equal(c.progress.covered.length,12);assert.ok(report.holdings.every(h=>h.evidenceIds.every(id=>c.evidence.some(e=>e.id===id))));});
