...
当前没有可启用真实交易的开关。允许的 POST 仅限本地订单预览/确认、已知任务取消、报告任务和策略停止新增信号；都不会调用券商写入。切换 LIVE 只是查看模式。SDK 的 clientId=0 会自动绑定外部订单，因此服务端禁止。
...
- P3 内部账本、一次性提交、发送前身份、管理订单事件、账户对账、撤改和本地订单界面已离线验证。源码 OrderExecutor 对真实 ibkr transport 明确拒绝；生产缺少可信完整风险 source。所有测试限额、策略和授权都是工程样本。原始 SDK 成交更正/手续费/UNKNOWN/改单回报已有离线验证；新 NativeDispatcher 已验证新订单原生序列化与最后发送边界，但默认关闭、未接生产许可签发或风险 source。撤改原生发送、生产接线与外部现金流对账仍是软件待办，真实验收另需 paper 范围授权。
