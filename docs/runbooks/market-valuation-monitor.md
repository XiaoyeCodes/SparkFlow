...
| 数据 | 口径 | 刷新频率 |
| --- | --- | --- |
| 标普500 市盈率（SPY） | World PE Ratio 的 SPY ETF 当前市盈率和月度历史，作为标普500估值参考；并非指数官方 TTM | 一小时 |
| 纳指100 市盈率（QQQ） | World PE Ratio 的 QQQ ETF 当前市盈率和月度历史，作为纳指100估值参考 | 一小时 |
| 前瞻盈利收益率 | `100 ÷ FactSet 未来12个月一致预期 P/E`，单位 %；解析最新 Earnings Insight 官方 PDF 第一页，同时记录报告发表日 | 一小时 |
| 10 年期美债 | FRED `DGS10`，美联储 H.15 固定期限名义收益率，单位 %，日频 | 一小时 |
| CNN 恐惧贪婪 | CNN Markets 发布的 0–100 指数快照和实际返回历史；不补造早期记录 | 一小时 |
...
文件仅保存在被 Git 忽略的本机 `.sparkflow` 目录。需要数据更新时替换 CSV 后重启开发服务，或等待一小时估值缓存到期。面板实际评分公式、权重、历史覆盖检查和缺失数据处理由 `server/ibkrValuationModel.ts` 管理。
...
模型 v1.4 使用 SPY 自身月度市盈率分位替代原标普500 TTM 分位，保持 35% 权重；QQQ 仍为独立参考。ETF 源页未明确盈利口径，因此卡片标为 P/E。已结束月份记为月末，当前月使用来源报价日；不插值为每日数据，不混接旧 Multpl/ChartRow 序列。

## 时间与异常处理

页面优先显示浏览器保存的最近快照；服务启动时恢复磁盘缓存，后台每小时检查并更新。过期缓存先返回再刷新，失败时保留旧数据并显示状态。缓存按规则版本与数据来源隔离，手动刷新仍受最小请求间隔限制。
