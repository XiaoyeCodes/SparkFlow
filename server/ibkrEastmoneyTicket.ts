import type { PaperContract, PaperQuote, TicketDataSource } from '../src/lib/ibkr/workbenchTypes.ts';
import type { JsonFetcher } from './ibkrMarket.ts';

// stock/get uses f162 (dynamic), f163 (static), f164 (TTM).
// Verified against https://quote.eastmoney.com/newstatic/build/vendor.js;
// f114/f115 are not PE fields on this endpoint.
const fields = 'f57,f58,f59,f43,f44,f45,f46,f47,f48,f60,f86,f162,f163,f164,f170';
...
// Zero is an upstream missing-value placeholder; negative earnings ratios are valid data.
const earningsRatio = (v: unknown) => (typeof v === 'number' || typeof v === 'string' && v.trim() !== '') && Number.isFinite(Number(v)) && Number(v) !== 0 ? String(v) : null;
...
export function newYorkTimestamp(value: unknown, now: number) {
...
export function parseTicketQuote(raw: any, contract: PaperContract, now = Date.now()): PaperQuote {
  ...
  return { ...contract, name: String(row.f58 || contract.name || contract.symbol), last, close,
    ...
    peDynamic:earningsRatio(row.f162),peStatic:earningsRatio(row.f163),peTtm:earningsRatio(row.f164),peSource:'东方财富',
    high: positive(row.f44), low: positive(row.f45), bid: null, ask: null,
    ...
    regularHours: null, nextOpen: null,
    detail: '每 5 秒刷新；上游未保证实时性，成交价格以 IBKR 回报为准。' };
}
...
export function parseTencentTicketQuote(raw: any, contract: PaperContract, now = Date.now()): PaperQuote {
  ...
  return { ...contract, name: String(row[46] || row[1] || contract.name || contract.symbol), last, close,
    open: positive(row[5]), volume: positive(row[6]), amount: positive(row[37]),
    // Tencent's quote center maps 39 to pe_ttm. US row[41] was cross-checked
    // against Tencent annual diluted EPS for MSFT, NVDA and TSLA (not the
    // mainland-China field layout). Tencent's current official mobile adapter
    // maps US row[65] to dynamic_ratio. See docs/plans/tencent-us-pe-fields.md.
    // Keep the legacy generic alias for optional enrichment of older callers.
    peRatio: earningsRatio(row[39]), peDynamic: earningsRatio(row[65]), peStatic: earningsRatio(row[41]), peTtm: earningsRatio(row[39]), peSource: '腾讯财经',
...
export class EastmoneyTicketQuotes {
  ...
  async quote(contract: PaperContract, source?: TicketDataSource) {
    ...
    const flight = (async () => {
      try {
        if (source === 'tencent') {
          const ticker = `us${tencentSymbol(contract.symbol)}`;
          const raw = await this.get(tencentTicketQuoteUrl(ticker));
          const value = parseTencentTicketQuote(raw, contract);
          value.source = '腾讯财经';
          value.detail = '腾讯财经参考行情；上游未保证实时性，成交价格以 IBKR 回报为准。';
        ...
        if (source === 'eastmoney' && matched.length === 0) throw new Error('东方财富行情暂不可用，请重试或切换腾讯。');
        let value = matched.length === 1 ? matched[0] : parseTencentTicketQuote(await this.get(tencentTicketQuoteUrl(`us${tencentSymbol(contract.symbol)}`)), contract);
        if (!source && matched.length === 1 && value.peDynamic == null && value.peTtm == null) {
          try {
            const supplement = parseTencentTicketQuote(await this.get(tencentTicketQuoteUrl(`us${tencentSymbol(contract.symbol)}`)), contract);
            // Enrich only the missing PE; retain primary prices, depth and quote time.
            if (supplement.peRatio != null) value = { ...value, peRatio: supplement.peRatio,
              peRatioSource: supplement.peSource, peRatioAsOf: supplement.asOf,
              peRatioFetchedAt: supplement.fetchedAt };
          } catch { /* Optional valuation failure must not discard a fresh primary quote. */ }
        }
        this.cache.set(key,{at:Date.now(),value});
        if (this.cache.size > 50) this.cache.delete(this.cache.keys().next().value!);
        return value;
      } catch (e) {
        if (cached) return {...cached.value,state:'stale',detail:`${cached.value.source}刷新失败，保留该来源上次报价；请留意报价时间。`};
        throw e;
