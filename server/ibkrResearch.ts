...
const action = z.object({symbol:text,action:z.enum(['hold','watch','increase','reduce']),horizon:z.enum(['short','long']),priority:z.enum(['high','medium','low']),rationale:text,executionWindow:text,riskControl:text,expectedImpact:text,counterEvidence:text,trigger:text,invalidation:text,targetWeight:z.number().min(0).max(1).nullable(),evidenceIds:ids}).strict();
