...
import type { DailyBriefResponse, DailyBriefSlot, DailyBriefSnapshot } from '../src/lib/dailyBriefTypes';
...
export function getDailyBriefWindow(now = new Date()): BriefWindow {
  ...
  return { date: local.date, slot: local.hour >= 18 ? 'evening' : 'morning' };
}
...
export function getNextDailyBriefRun(now = new Date()) {
  ...
  const targetHour = hour < 8 ? 8 : hour < 18 ? 18 : 32;
  const shanghaiAsUtc = Date.UTC(Number(parts.year), Number(parts.month) - 1, Number(parts.day), targetHour, 0, 0);
...
function isSnapshot(value: unknown): value is DailyBriefSnapshot {
  ...
  return Boolean(item && item.version === 1 && /^\d{4}-\d{2}-\d{2}$/.test(item.date || '')
    && ['morning', 'evening'].includes(item.slot || '') && item.summary && Array.isArray(item.markets));
}
...
function isScheduledWindowComplete(snapshot: DailyBriefSnapshot) {
  ...
  return snapshot.slot === 'evening' ? generated.hour >= 18 : generated.hour >= 8 && generated.hour < 18;
}
...
export function createDailyBriefService(options: { stateDir: string; generate: GenerateBrief; now?: () => Date }) {
  ...
  async function cleanupHistory() {
    ...
    const dated = entries.filter((entry) => entry.isDirectory() && /^\d{4}-\d{2}-\d{2}$/.test(entry.name)).sort().reverse();
    await Promise.all(dated.slice(HISTORY_DAYS).map((entry) => rm(path.join(root, entry.name), { recursive: true, force: true })));
  }

  async function generate(window: BriefWindow, force = false): Promise<DailyBriefResponse> {
    ...
    if (inMemory && (!force || isScheduledWindowComplete(inMemory))) {
      return { snapshot: inMemory, cache: { hit: true, key: locations.key, generated: false } };
    }
    ...
    if (cached && cached.date === window.date && cached.slot === window.slot) {
      if (!force || isScheduledWindowComplete(cached)) {
        ...
        return { snapshot: cached, cache: { hit: true, key: locations.key, generated: false } };
      }
    }
    ...
    if (!lockHandle) {
      ...
      if (peer) {
        ...
        return { snapshot: peer, cache: { hit: true, key: locations.key, generated: false } };
      }
      const stale = await readSnapshot(locations.file);
      if (stale) {
        memory.set(locations.key, stale);
        return { snapshot: stale, cache: { hit: true, key: locations.key, generated: false } };
      }
      throw new Error('每日简报正在由另一任务生成，请稍后重试');
    }
    try {
      const snapshot = await options.generate(window);
      if (!isSnapshot(snapshot) || snapshot.date !== window.date || snapshot.slot !== window.slot) throw new Error('每日简报生成结果无效');
      ...
      void cleanupHistory();
      return { snapshot, cache: { hit: false, key: locations.key, generated: true } };
    } finally {
  ...
  function get(window = getDailyBriefWindow(clock()), force = false) {
    ...
    const request = generate(window, force).finally(() => running.delete(key));
  ...
  function schedule(onError: (error: unknown) => void = () => undefined) {
    ...
    const arm = () => {
      ...
      timer = setTimeout(() => {
        void get(getDailyBriefWindow(clock()), true).catch(onError).finally(arm);
      }, delay);
      timer.unref?.();
    };
    arm();
