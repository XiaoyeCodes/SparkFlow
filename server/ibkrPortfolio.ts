...
export function gatewayPerformance(points: PerformancePoint[], currency: string|null, mode: 'live'|'paper'): PortfolioPerformance {
 const account=mode==='paper'?'模拟盘':'实盘';
 return {...localPerformance(points,currency),source:`IB Gateway ${account} · 本地净值快照`,note:`当前数据来自 IB Gateway ${account}。历史曲线由 SparkFlow 按日保存净值快照；净值变化包含出入金，不作为投资收益率或盈亏金额。`};
}
